...
const nudges = [
  ...
  },
  {
    ...
    message: "We noticed you tend to shop online between 11PM-2AM. These purchases are 60% more likely to be returned.",
    time: "5 days ago",
    type: "warning",
    category: "Behavior",
  ...
  },
  {
    ...
    message: "Your entertainment spending is down 15% this month. Great job sticking to your budget!",
    time: "1 week ago",
    type: "success",
    category: "Behavior",
...
const nudgeSettings = [
  {
    ...
    description: "Get notified when your spending in a category exceeds your usual patterns",
    enabled: true,
  },
  {
    id: 2,
    name: "Budget Alerts",
    description: "Receive alerts when you're approaching or have exceeded your budget limits",
    enabled: true,
  },
  {
    id: 3,
  ...
  },
  {
    ...
    description: "Receive suggestions for potential savings based on your spending",
    enabled: true,
  },
  {
    id: 5,
    name: "Subscription Reminders",
    description: "Get reminded before subscriptions renew, especially for unused services",
    enabled: true,
  },
]
...
export default function NudgesPage() {
  ...
  const toggleDelivery = (id) => {
    setDelivery(delivery.map((pref) => (pref.id === id ? { ...pref, enabled: !pref.enabled } : pref)))
  }

  const filteredNudges = activeTab === "all" ? nudges : nudges.filter((nudge) => nudge.type === activeTab)
      ...
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            ...
            <p className="text-gray-400">Personalized notifications to help you make better financial decisions</p>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="w-full">
              <div className="flex justify-between items-center mb-6">
                ...
                <Badge className="bg-gray-800 text-gray-300 border-gray-700">{filteredNudges.length} Nudges</Badge>
              </div>
                      ...
                      <div className="flex space-x-4">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-start mb-2">
                            <h4 className="text-lg font-medium text-white">{nudge.title}</h4>
                            <Badge
                              className={`${
                                nudge.type === "warning"
                          ...
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-500">{nudge.time}</span>
                            <div className="flex space-x-2">
                              {nudge.type === "warning" || nudge.type === "info" ? (
                                <>
                                  ...
                                  >
                                    <CheckCircle className="h-4 w-4 mr-1" /> Take Action
                                  </Button>
                                </>
                              ) : (
                                <Button
                ...
                {filteredNudges.map((nudge) => (
                  ...
                  >
                    <CardContent className="p-6">
                      <div className="flex space-x-4">
                        <div className="flex-shrink-0 mt-0.5">
                          <div className="p-2 rounded-full bg-yellow-500/20">{nudge.icon}</div>
                        </div>
                        <div className="flex-1">
                          <div className="flex justify-between items-start mb-2">
                            <h4 className="text-lg font-medium text-white">{nudge.title}</h4>
                ...
                {filteredNudges.map((nudge) => (
                  ...
                  >
                    <CardContent className="p-6">
                      <div className="flex space-x-4">
                        <div className="flex-shrink-0 mt-0.5">
                          <div className="p-2 rounded-full bg-blue-500/20">{nudge.icon}</div>
                        </div>
                        <div className="flex-1">
                          <div className="flex justify-between items-start mb-2">
                            <h4 className="text-lg font-medium text-white">{nudge.title}</h4>
                            <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30">{nudge.category}</Badge>
                          </div>
                          <p className="text-gray-300 mb-4">{nudge.message}</p>
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-500">{nudge.time}</span>
                            <div className="flex space-x-2">
                              ...
                              <Button size="sm" className="h-8 px-3 text-sm bg-blue-600 hover:bg-blue-500 text-white">
                                <CheckCircle className="h-4 w-4 mr-1" /> Take Action
                              </Button>
                            </div>
                          </div>
                ...
                {filteredNudges.map((nudge) => (
                  ...
                  >
                    <CardContent className="p-6">
                      <div className="flex space-x-4">
                        <div className="flex-shrink-0 mt-0.5">
                          <div className="p-2 rounded-full bg-green-500/20">{nudge.icon}</div>
                        </div>
                        <div className="flex-1">
                          <div className="flex justify-between items-start mb-2">
                            <h4 className="text-lg font-medium text-white">{nudge.title}</h4>
                          ...
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-500">{nudge.time}</span>
                            <Button size="sm" className="h-8 px-3 text-sm bg-green-600 hover:bg-green-500 text-white">
                              <CheckCircle className="h-4 w-4 mr-1" /> Acknowledge
                            </Button>
                          </div>
                        </div>
          ...
          <div className="space-y-8">
            <Card className="bg-gray-800 border-gray-700">
              <CardHeader>
                ...
                <CardDescription className="text-gray-400">Customize which types of nudges you receive</CardDescription>
              </CardHeader>
            ...
            <Card className="bg-gray-800 border-gray-700">
              <CardHeader>
                ...
                <CardDescription className="text-gray-400">Choose how you want to receive nudges</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {delivery.map((pref) => (
                  <div key={pref.id} className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div className="p-2 rounded-full bg-gray-700">{pref.icon}</div>
                      <div className="space-y-0.5">
            ...
            <Card className="bg-yellow-500/10 border border-yellow-500/30">
              ...
              <CardContent className="space-y-4">
                <p className="text-gray-300 text-sm">
                  Our AI analyzes your transaction data in real-time to provide personalized nudges that help you:
                </p>
                <ul className="space-y-2">
                  <li className="flex items-start">
                    <span className="text-yellow-500 mr-2">•</span>
                    <span className="text-gray-300 text-sm">Identify impulse spending before it happens</span>
                  </li>
                  <li className="flex items-start">
                    <span className="text-yellow-500 mr-2">•</span>
                    <span className="text-gray-300 text-sm">Stay on track with your budgets and savings goals</span>
                  </li>
                  <li className="flex items-start">
                    <span className="text-yellow-500 mr-2">•</span>
                    <span className="text-gray-300 text-sm">Recognize and reinforce positive financial behaviors</span>
                  </li>
                  <li className="flex items-start">
                    <span className="text-yellow-500 mr-2">•</span>
                    <span className="text-gray-300 text-sm">
                      Make informed decisions about subscriptions and recurring expenses
                    </span>
                  </li>
                </ul>
              </CardContent>
