...
const loadContract = async () => {
  if (!codiContract) {
    console.log("⏳ Loading SIT contract...");
    ...
    console.log("✅ SIT Contract loaded");
  }
  return codiContract;
};
...
export const getBalance = async (account) => {
  try {
    ...
    console.log(`💰 ${displayBalance} SIT`);
...
export const buyCodi = async (to, amount) => {
  try {
    ...
    console.log(`✅ Sent ${amount} SIT to ${sanitizedTo}`);
    return txResult;
  } catch (err) {
    console.error("❌ Error sending SIT:", err);
    return null;
  }
};